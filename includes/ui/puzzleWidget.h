...
class PuzzleWidget : public QGraphicsView
{
protected:
	void resizeEvent(QResizeEvent *event) override
	{
	...
	public:
		PuzzleWidget(const int& size, QObject* paret=nullptr) :
			QGraphicsView(), tiles_(0), size_(size), numOfTiles_(size * size)
		{
			//![1]
...
//![9]
    ...
    /* show(); */
		...
		/* 	QState* state1 = createGeometryState(group); /1* new createGeometryState() *1/ */
